...
let goblin=createCharacter(randomPosition(CANVAS_WIDTH,GOBLIN_SIZE,PADDING),randomPosition(CANVAS_HEIGHT,GOBLIN_SIZE,PADDING),GOBLIN_SIZE,GOBLIN_SIZE);
//! Cargar imágenes
...
function handleKeyDown(e,character){
  ...
  switch (e.key){
    case "ArrowUp":
      character.y=Math.max(character.y-character.speed,PADDING);
      break;
    case "ArrowDown":
      character.y=Math.min(character.y+character.speed,CANVAS_HEIGHT-character.height-PADDING);
      break;
    case "ArrowLeft":
      character.x=Math.max(character.x-character.speed,PADDING);
      break;
    case "ArrowRight":
      character.x=Math.min(character.x+character.speed,CANVAS_WIDTH-character.width-PADDING);
      break;
  }
...
//! función de dibujar personaje
function drawCharacter(img,character){
  ctx.drawImage(img,character.x,character.y,character.width,character.height);
}
//! Función de mostrar puntaje
function drawScore(){
  ctx.font="20px 'MedievalSharp',sans-serif"; // Fuente del marcador
  ctx.fillStyle="white";
  ctx.fillText("Goblins Caught: "+score,10,30);
}
//! Función que une el fondo,el heroe y el goblin
function draw(){
...
//! Opción para que el juego sea infinito
function gameLoop(){
